...
palau_polygons <- palau_polygons %>% 
  addPolygons(data=PNMS_outline, color=PNMS_color, opacity=1, weight=1, fillOpacity=0.2) %>% 
  addLegend(
...
ui <- fluidPage(
  ...
  navset_tab(
    ...
    tabPanel("Visualizations",
             fileInput("uploaded_enviro_file", "Upload environmental data as .csv file with proper formatting"), #creates file input option
             fileInput("uploaded_edna_file", "Upload eDNA data as .csv file with proper formatting"), #creates file input option
  ...
  selectInput("env_var", "Environmental Variable:",
              c("Salinity" = "Sal.", "Temperature" = "Temp...degC.", "pH" = "pH", "chlorophyll A" = "Chl.a..??g.l.")),
